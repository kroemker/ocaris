...
- If the primary candidate turns out to be unworkable (no stable API, ToS blocks it, structure too unstable), fall back to investigating `zelda-64-mods.fandom.com`'s MediaWiki `action=query` API instead, and document _that_ schema.
